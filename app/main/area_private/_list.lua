...
if app.session.member_id then
  areas_selector:add_field({ "(SELECT COUNT(*) FROM issue LEFT JOIN direct_voter ON direct_voter.issue_id = issue.id AND direct_voter.member_id = ? WHERE issue.area_id = area.id AND issue.fully_frozen NOTNULL AND issue.closed ISNULL AND direct_voter.member_id ISNULL)", app.session.member.id }, "issues_to_vote_count")
  ...
  :left_join("delegation", nil, {"delegation.truster_id = ? AND delegation.area_id = area.id AND delegation.scope = 'area'", app.session.member_id})
...
areas_selector:join("issue", nil, "area.unit_id = unit.id AND NOT unit.public")
...
if #areas == 0 then
  ...
  for  i, area in ipairs(areas) do
    execute.view { module = "area_private", view = "_list_entry", params = { area = area, member = member, create = create } }
  end
end
