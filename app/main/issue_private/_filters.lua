...
if for_member then
  ...
  if not for_unit and not for_area then
    ...
    filter[#filter + 1] = {
      ...
      selector_modifier = function(selector)
        ...
        selector:join("privilege", nil, { "privilege.unit_id = area.unit_id AND privilege.member_id = ? AND privilege.voting_right", for_member.id })
      end
    }
  end
  ...
  if not for_area then
    filter[#filter + 1] = {
      ...
      label = _ "My areas",
      selector_modifier = function(selector)
        selector:join("membership", nil, { "membership.area_id = issue.area_id AND membership.member_id = ?", for_member.id })
      end
    }
  end
  if for_area then
  ...
  filter[#filter + 1] = {
    ...
    label = _ "Initiated",
    selector_modifier = function(selector)
      selector:add_where({ "EXISTS (SELECT 1 FROM initiative JOIN initiator ON initiator.initiative_id = initiative.id AND initiator.member_id = ? AND initiator.accepted WHERE initiative.issue_id = issue.id)", for_member.id })
    end
  }
  filter[#filter + 1] = {
    name = "supported",
  ...
  if filter_interest ~= "any" and filter_interest ~= nil and (filter_interest == "issue" or filter_interest == "supported" or filter_interest == "potentially_supported" or
    (filter_interest == 'voted' and state ~= 'open')) then

    local function add_default_joins(selector)
      selector:left_join("interest", "filter_interest", { "filter_interest.issue_id = issue.id AND filter_interest.member_id = ? ", for_member.id })
      selector:left_join("direct_interest_snapshot", "filter_interest_s", { "filter_interest_s.issue_id = issue.id AND filter_interest_s.member_id = ? AND filter_interest_s.event = issue.latest_snapshot_event", for_member.id })
      selector:left_join("delegating_interest_snapshot", "filter_d_interest_s", { "filter_d_interest_s.issue_id = issue.id AND filter_d_interest_s.member_id = ? AND filter_d_interest_s.event = issue.latest_snapshot_event", for_member.id })
    end
        ...
        selector_modifier = function(selector)
          ...
          elseif filter_interest == "potentially_supported" then
            selector:add_where({
                ...
                "END OR EXISTS(SELECT 1 FROM direct_supporter_snapshot WHERE direct_supporter_snapshot.event = issue.latest_snapshot_event AND direct_supporter_snapshot.issue_id = issue.id AND direct_supporter_snapshot.member_id = filter_d_interest_s.delegate_member_ids[array_upper(filter_d_interest_s.delegate_member_ids,1)] AND NOT direct_supporter_snapshot.satisfied)", for_member.id, for_member.id, for_member.id
              })

          elseif filter_interest == "voted" then
            selector:add_where({ "EXISTS(SELECT 1 FROM direct_voter WHERE direct_voter.issue_id = issue.id AND direct_voter.member_id = ?) OR (issue.closed NOTNULL AND EXISTS(SELECT 1 FROM delegating_voter WHERE delegating_voter.issue_id = issue.id AND delegating_voter.member_id = ?)) ", for_member.id, for_member.id })
          end
        end
      },
      {
        ...
        selector_modifier = function(selector)
          ...
          if filter_interest == "supported" then
            selector:add_where({
                ...
                "ELSE " ..
                "EXISTS(SELECT 1 FROM direct_supporter_snapshot WHERE direct_supporter_snapshot.event = issue.latest_snapshot_event AND direct_supporter_snapshot.issue_id = issue.id AND direct_supporter_snapshot.member_id = ? AND direct_supporter_snapshot.satisfied) " ..
                "END", for_member.id, for_member.id
              })
          ...
          elseif filter_interest == "voted" then
            selector:add_where({ "EXISTS(SELECT 1 FROM direct_voter WHERE direct_voter.issue_id = issue.id AND direct_voter.member_id = ?) ", for_member.id })
          end
        end
      },
      {
        ...
        selector_modifier = function(selector)
          ...
          if filter_interest == "supported" then
            selector:add_where({
                "EXISTS(SELECT 1 FROM direct_supporter_snapshot WHERE direct_supporter_snapshot.event = issue.latest_snapshot_event AND direct_supporter_snapshot.issue_id = issue.id AND direct_supporter_snapshot.member_id = filter_d_interest_s.delegate_member_ids[array_upper(filter_d_interest_s.delegate_member_ids,1)] AND direct_supporter_snapshot.satisfied)", for_member.id
              })

          elseif filter_interest == "potentially_supported" then
            selector:add_where({
                "EXISTS(SELECT 1 FROM direct_supporter_snapshot WHERE direct_supporter_snapshot.event = issue.latest_snapshot_event AND direct_supporter_snapshot.issue_id = issue.id AND direct_supporter_snapshot.member_id = filter_d_interest_s.delegate_member_ids[array_upper(filter_d_interest_s.delegate_member_ids,1)] AND NOT direct_supporter_snapshot.satisfied)", for_member.id
              })
          elseif filter_interest == "voted" then
            selector:add_where({ "issue.closed NOTNULL AND EXISTS(SELECT 1 FROM delegating_voter WHERE delegating_voter.issue_id = issue.id AND delegating_voter.member_id = ?) ", for_member.id })
          end
        end
      }
    }
...
if state == 'open' and app.session.member and for_member and for_member.id == app.session.member_id and (param.get_all_cgi()["filter"] == "frozen") then
  filters[#filters + 1] = {
    name = "filter_voting",
    {
      name = "any",
    ...
    },
    {
      ...
      label = _ "Not voted",
      selector_modifier = function(selector)
        selector:left_join("direct_voter", nil, { "direct_voter.issue_id = issue.id AND direct_voter.member_id = ?", for_member.id })
        selector:add_where("direct_voter.member_id ISNULL")
        selector:left_join("non_voter", nil, { "non_voter.issue_id = issue.id AND non_voter.member_id = ?", for_member.id })
    ...
    },
    {
      ...
      label = _ "Voted",
      selector_modifier = function(selector)
        selector:join("direct_voter", nil, { "direct_voter.issue_id = issue.id AND direct_voter.member_id = ?", for_member.id })
      end
    },
  }
end
